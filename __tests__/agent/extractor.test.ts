...
describe("extractProfileFromResume", () => {
  ...
  it("returns success:true with an empty object — every field in the schema is optional", async () => {
  ...
  it("returns a safe failure when a work_experience entry is missing a required field", async () => {
    // "current" (boolean) is required on WorkExperienceEntrySchema — omitting it
    // must fail shape validation rather than silently coercing.
    mockCompletion(
      JSON.stringify({
        work_experience: [
          {
  ...
  it("rejects an experience_level value outside the allowed enum", async () => {
    mockCompletion(JSON.stringify({ experience_level: "executive" }));
  ...
  it("calls the model when resume text is exactly at the 100-character threshold", async () => {
    mocks.getText.mockResolvedValue({ text: "x".repeat(100) });
    mockCompletion(JSON.stringify(VALID_EXTRACTION));
  ...
  it("bounds the resume text sent to the model to MAX_RESUME_TEXT_CHARS (18000)", async () => {
    mocks.getText.mockResolvedValue({ text: "a".repeat(30000) });
    mockCompletion(JSON.stringify(VALID_EXTRACTION));

    await extractProfileFromResume(buffer);

    const sentContent = mocks.create.mock.calls[0][0].messages[1].content as string;
    const sentResumeText = sentContent.replace(/^RESUME TEXT:\n/, "");
    expect(sentResumeText.length).toBe(18000);
  });
});
