...
describe("researchCompany — synthesis request construction and retry mechanics", () => {
...
describe("researchCompany — synthesis shape validation", () => {
  it("recovers when the JSON is valid but missing the required whyThisRole field", async () => {
    const missingField = { ...VALID_DOSSIER_JSON } as Record<string, unknown>;
    delete missingField.whyThisRole;
    mockCompletion(JSON.stringify(missingField), "stop");
    mockCompletion(JSON.stringify(VALID_DOSSIER_JSON));
  ...
  it("recovers when the JSON is valid but missing the required companyOverview field", async () => {
    const missingField = { ...VALID_DOSSIER_JSON } as Record<string, unknown>;
    delete missingField.companyOverview;
    mockCompletion(JSON.stringify(missingField), "stop");
    mockCompletion(JSON.stringify(VALID_DOSSIER_JSON));
  ...
  it("defaults optional array fields to [] when the model omits them entirely", async () => {
    const minimalDossier = {
      companyOverview: "Minimal overview.",
      whyThisRole: "Minimal reason.",
    };
    mockCompletion(JSON.stringify(minimalDossier));

    const { dossier } = await researchCompany("job-1", "user-1");

    expect(dossier.companyOverview).toBe("Minimal overview.");
    expect(dossier.whyThisRole).toBe("Minimal reason.");
