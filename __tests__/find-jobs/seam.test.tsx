...
describe("Find Jobs UI/API seam — error handling", () => {
  ...
  it("renders the network-error banner with error styling, not the success style", async () => {
    // Regression: setSearchStatus({ message, isError: true }) must actually set
    // isError so SearchControls renders the red/error variant, not the default
    // success-green banner used for a normal completed search.
    global.fetch = vi.fn().mockRejectedValueOnce(new Error("Network error"));
    ...
    const banner = screen.getByText("Search failed. Please try again.").closest("div");
    expect(banner).toHaveClass("text-error");
  });

  it("a successful search after a prior network error clears the error banner", async () => {
    global.fetch = vi
      .fn()
      ...
      .mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            success: true,
            jobs: [makeJob("recovered-1", "Recovered Role", "Recovered Corp")],
            jobsFound: 1,
            newJobs: 1,
            successMessage: "Found 1 jobs and saved 1 new jobs.",
          }),
      });
    ...
    await clickFindJobs();
    await waitFor(() => {
      expect(screen.getByText(/found 1 jobs/i)).toBeInTheDocument();
    });
