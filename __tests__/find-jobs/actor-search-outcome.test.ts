...
describe("classifySearchPage", () => {
  ...
  it("login redirect URL takes priority even when cards are present", () => {
    // Defensive: a stale cardCount from a previous page combined with a login
    // redirect must still be classified as login-redirect, not "ok".
    expect(
      classifySearchPage({ ...base, url: "https://hk.jobsdb.com/login", cardCount: 5 }),
    ).toBe("login-redirect");
  });

  it("known empty-state selector wins even when the text fallback also matched", () => {
    expect(
      classifySearchPage({ ...base, hasKnownEmptySelector: true, hasEmptyTextFallback: true }),
    ).toBe("legit-empty");
  });
});
...
describe("shouldFailRun", () => {
  ...
  it("page-1 ok with jobs pushed → false (normal successful run)", () => {
    expect(shouldFailRun({ page1Outcome: "ok", jobsPushed: 12 })).toBe(false);
  });

  it("page-1 legit-empty with jobs pushed → false (defensive; shouldn't happen but must not fail)", () => {
    expect(shouldFailRun({ page1Outcome: "legit-empty", jobsPushed: 1 })).toBe(false);
  });
});
