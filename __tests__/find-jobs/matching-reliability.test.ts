...
describe("POST /api/agent/find — zero-result and legacy-signature edge cases", () => {
  it("never calls scoreJobs when the actor discovers zero jobs", async () => {
    const updateSpy = vi.fn();
    const insertSpy = vi.fn();
    ...
    expect(vi.mocked(scoreJobs)).not.toHaveBeenCalled();
    expect(updateSpy).not.toHaveBeenCalled();
    expect(insertSpy).not.toHaveBeenCalled();
  });

  it("does not treat a job with the legacy match_reason but a non-empty matched_skills as needing rescore", async () => {
    const sourceUrl = "https://hk.jobsdb.com/job/not-quite-legacy";
    const existing = makeExistingJob({
      id: "not-quite-legacy",
      source_url: sourceUrl,
      match_score: 0,
      match_reason: UNSCORED_MATCH_REASON,
      // A real fallback-scored row always has empty skill arrays — a
      // non-empty one here means this is coincidentally not the legacy
      // sentinel shape, so it must not be rescored.
      matched_skills: ["TypeScript"],
      missing_skills: [],
    });
    const actorJobs = [makeActorJob(sourceUrl)];
  ...
  it("treats a legacy fallback row with undefined skill arrays as still needing rescore", async () => {
    const sourceUrl = "https://hk.jobsdb.com/job/legacy-undefined-skills";
    const existing = makeExistingJob({
      id: "legacy-undefined-skills",
      source_url: sourceUrl,
      match_score: 0,
      match_reason: UNSCORED_MATCH_REASON,
      matched_skills: undefined as unknown as string[],
      missing_skills: undefined as unknown as string[],
    });
    const actorJobs = [makeActorJob(sourceUrl)];
    const updateSpy = vi.fn();
    const insertSpy = vi.fn();
    ...
    vi.mocked(discoverJobsDbJobs).mockResolvedValue(actorJobs);
    vi.mocked(scoreJobs).mockResolvedValue([realScore(77)]);
    vi.mocked(createInsforgeServer).mockResolvedValue(
      buildInsforgeMock({
        existingJobsResult: { data: [existing], error: null },
        insertResult: { data: [], error: null },
        updateResultsById: new Map([
          [
            "legacy-undefined-skills",
            { data: { ...existing, match_score: 77 }, error: null },
          ],
        ]),
        updateSpy,
        insertSpy,
    ...
    expect(updateSpy).toHaveBeenCalledWith(
      "legacy-undefined-skills",
      expect.objectContaining({ match_score: 77 }),
    );
  });
});
