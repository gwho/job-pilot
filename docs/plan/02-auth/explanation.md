...
6. The Route Handler's *response* (not the request) gets `insforge_access_token` (readable) and `insforge_refresh_token` (httpOnly) set via `responseCookies`, the verifier cookie is deleted, and the response redirects to `/dashboard`. In a Route Handler, `responseCookies` must be the outgoing `NextResponse.cookies` writer.
7. On the next request, `proxy.ts` runs `updateSession()` first (refreshing the access token from the refresh token if it's expired), then uses the returned `accessToken` to decide whether the user should reach the protected route.

## 7. How `updateSession` avoids a stale-token race in Server Components

`proxy.ts` runs *before* the route's Server Components render, on every matched request. It calls `updateSession()` synchronously in that request's lifecycle and writes any refreshed token onto the *response* cookies before passing the request through with `NextResponse.next()`. The auth decision must use `updateSession()`'s returned `accessToken`, not only the original request cookie, because the original access token may have been missing or expired before the refresh succeeded. The race this avoids is the opposite ordering: if a Server Component rendered first and only refreshed afterward, the user could see a logged-out flash or a failed query against an expired token even though a valid refresh token existed.

## 8. What the completion pass corrected

The follow-up audit in `docs/diff/02-auth/README.md` found that the first implementation had the right file structure but not the right response-cookie wiring in two Route Handlers. `createAuthActions()` writes login/logout cookie changes through `responseCookies`, so the callback and API sign-out routes now create the outgoing `NextResponse` first and pass that response's `.cookies` object into the SDK. The same pass updated `proxy.ts` to trust `updateSession()`'s returned `accessToken`, added the friendly `/login?error=oauth` state, and updated project docs so future auth work copies the corrected pattern.
