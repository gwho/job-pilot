...
- **2026-07-23 — AI matching score/skill regression recovery**: Backend-only fix in `agent/job-matcher.ts` and `app/api/agent/find/route.ts`. No UI components, classes, or visual states changed. `components/job-details/JobInfo.tsx`, `components/job-details/MatchScore.tsx`, `components/find-jobs/JobsTable.tsx`, and `components/find-jobs/FindJobsClient.tsx` already fully guard on `match_score != null` (badge/bar hidden, "—" placeholder, high/low filters and score-sort already exclude/sink null), and `app/dashboard/page.tsx`'s average/distribution calculations already filter `match_score IS NOT NULL` — all existing behavior for the "unscored" state this fix now actually produces. No code changes were needed in any of these files.
